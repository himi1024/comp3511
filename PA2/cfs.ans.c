...
void init_cfs_process() {
	...
	for(int i=0; i < num_process;i++){
		...
		total_weight += process[i].weight;
	}
	for(int	i=0; i < num_process;i++){
		process[i].time_slice = calculate_per_process_time_slice(process[i].weight,sched_latency,total_weight);
...
void run_cfs_scheduling() {
    ...
    // loop until all process is finished
    while(num_process != finish_process_count){
	//find the process with lowest vruntime
	int run = 0;
	for(int i=0; i < num_process; i++){
		...
		if(process[run].remain_time == 0)
			...
			if(process[run].vruntime >  process[i].vruntime)
				run = i;
		}
	}
	
	//update 
		if(process[run].remain_time <= process[run].time_slice){
			//end_time += process[run].remain_time;
			process[run].vruntime = calculate_new_vruntime(process[run].vruntime, process[run].remain_time, process[run].weight);  		
			finish_process_count +=1;
			gantt_chart_append_item(run, process[run].remain_time);
			process[run].remain_time = 0;
		}
		else{
			//end_time += process[run].time_slice;
			process[run].vruntime = calculate_new_vruntime(process[run].vruntime, process[run].time_slice, process[run].weight);
			//process[run].remain_time -= process[run].time_slice;		
			gantt_chart_append_item(run, process[run].time_slice);
			process[run].remain_time -= process[run].time_slice;
		}
